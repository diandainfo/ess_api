...
||lid|integer|限制整型，线路编号|
||lld|string|lastLoginDate，最后一次登录日期，格式化为"2016-01-01"，会进行格式校验|
...
|phone  |否|string |手机号|
|state  |否|int     |状态|
|capacity  |否|int     |店铺大小(容量) 10:小型(20平方米以下) 20:中型(20平方米-100平方米) 30:大型(100平方米)|
|type|否|int|店铺类型(1:烟酒店 2:便利店 3:综合性超市 4:果蔬店 5:餐饮店 99:其他)|
|lid|否|int|限制整型，线路编号|
...
|lid|否|int|-|线路编号，整型匹配|
|lld|否|string|-|最后一次登录时间的筛选，返回 `<lld` 的店铺;**本周、月未登录店铺**的筛选|
|key|否|string|-|搜索关键词，可填写：店铺名称、店主姓名、手机号|
|sids|否|\<Array\>int|-|店铺编号列表，整型数组，如[1,2,3,4]，用于查询时批量筛选店铺|
